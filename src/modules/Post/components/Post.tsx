  ...
  const handleFavorite = () => {
    ...
    if (item.isLike === 'true') {
      handleInteraction('dislike', params)
        .then((res) => {
          if (res.msg === 'success') setLikeNumChange(item, 0);
        })
        .catch((err) => {
          console.log(err);
        });
      ...
      handleInteraction('like', params)
        .then((res) => {
          if (res.msg === 'success') setLikeNumChange(item, 1);
        })
        .catch((err) => {
          console.log(err);
        });
      ...
      <View className="content">
        <View className="title-container">
          <View className="post-user">
            <Image className="avatar" src={item.userInfo.avatar} mode="widthFix"></Image>
