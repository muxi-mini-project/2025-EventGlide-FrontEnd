...
const ActivityAddRules: React.FC<{ setShowAddRules: (show: boolean) => void; }> = memo(({ ...props }) => {
    return (
        <View className="rule-window">
            <View className="rule-window-background"></View>
            <View className="rule-window-container">
                <View
                    className="rule-window-close"
                    onClick={() => props.setShowAddRules(false)}
                >
                    ×
                </View>
                ...
                <View className="rule-window-content">
                    ...
                    <Text>
                        4、“活动地点”为自行填写，请填写尽量详细。
                    </Text>
                    <Text>
                        5、请根据实际情况填写“是否需要报名”，若需要报名请详细说明报名方式。
                    </Text>
                    <Text>
                        6、若勾选“是否外校可见”为“是”，则该活动也将展示在其他学校的首页中。
                    </Text>
                    <Text>
                        7、所有活动的申报均需进行申报人身份认证。若活动承办方为非个人，则需进行1人身份认证以及活动申报表提交；若活动承办方为个人，则需进行至少3人身份认证。
                    </Text>
                    <Text>
                        8、确认所有信息填写无误后即可发布审核，审核通过后将在首页进行展示。
                    </Text>
                </View>
            </View>
        </View>
